...
//const RENT_PROGRAM = new PublicKey(process.env.SYSTEM_RENT || 'SysvarRent111111111111111111111111111111111');
...
/** Construit et envoie une transaction d'achat pump.fun */
export async function sendBuyTransaction(connection: Connection, wallet: Keypair, tokenAddress: string, solAmount: number, slippage = 10, priorityFee = 0.0001) {

    try {
        // Envoi de la transaction
        const signature = await pumpFunBuy(connection, null, wallet, { mint: new PublicKey(tokenAddress) }, solAmount, slippage, priorityFee);
