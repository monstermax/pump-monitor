...
export type TokenMetadataResponse = {
    metadata: Omit<TokenMetadata, 'uri'>,
    metadataUri: string,
}
...
export async function createTokenMetadata(create: CreateTokenMetadata): Promise<TokenMetadataResponse> {
    // Validate file
    const file = await create.file();
