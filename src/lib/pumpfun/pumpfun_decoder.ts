...
export function parsePumpTransaction(txData: VersionedTransactionResponse): PumpTokenInfo | TradeInfo {
    // Déterminer le type d'instruction
    const instructionType = detectInstructionType(txData);

    // En fonction du type, appeler le bon parseur
    if (instructionType === 'create') {
        const tokenInfo = parseCreateInstruction(txData);
    ...
    throw new Error('Type d\'instruction non reconnu');
}



/** Détecte le type d'instruction principal dans la transaction */
function detectInstructionType(txData: VersionedTransactionResponse): string {
...
function parseCreateInstruction(txData: VersionedTransactionResponse): PumpTokenInfo {
...
function parseBuyInstruction(txData: VersionedTransactionResponse): TradeInfo {
    const accounts = txData.transaction.message.staticAccountKeys.map(key => key.toString());
...
function parseSellInstruction(txData: VersionedTransactionResponse): TradeInfo {
...
function hasInitialBuy(logMessages: string[]): boolean {
...
function calculatePumpPrice(virtualSolReserves: number, virtualTokenReserves: number): string {
...
function parseTradeEvent(tradeEventLog: string): TradeLog {
...
function extractMetadataFromProgramData(programDataLog: string): {
    name: string;
    symbol: string;
    uri: string;
    rawData?: Uint8Array;
...
if (require.main === module) {
    try {
        ...
        const tokenInfo = parsePumpTransaction(txData);

        if ('tokenName' in tokenInfo) {
            // MINT
